...
from app.services.kpi import daily_analytics, dashboard_metrics, funnel_metrics, manager_kpi, monthly_analytics
...
def login(payload: LoginIn, db: Session = Depends(get_db)):
    ...
    if not login_value:
        raise HTTPException(status_code=422, detail="Передайте email или username")
...
def demo_users(db: Session = Depends(get_db)):
    ...
    return [{"email": u.email, "role": u.role.name} for u in rows]
...
def analytics_dashboard(date_from: date | None = None, date_to: date | None = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    manager_id = user.id if user.role.name == "manager" else None
    return dashboard_metrics(db, manager_id=manager_id, date_from=date_from, date_to=date_to)


@router.get("/analytics/funnel")
def analytics_funnel(date_from: date | None = None, date_to: date | None = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    manager_id = user.id if user.role.name == "manager" else None
    return funnel_metrics(db, manager_id=manager_id, date_from=date_from, date_to=date_to)


@router.get("/analytics/kpi")
def analytics_kpi(date_from: date | None = None, date_to: date | None = None, _: User = Depends(require_roles("head", "analyst")), db: Session = Depends(get_db)):
    return manager_kpi(db, date_from=date_from, date_to=date_to)
...
def analytics_monthly(date_from: date | None = None, date_to: date | None = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    manager_id = user.id if user.role.name == "manager" else None
    return monthly_analytics(db, manager_id=manager_id, date_from=date_from, date_to=date_to)


@router.get("/analytics/daily")
def analytics_daily(date_from: date | None = None, date_to: date | None = None, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    manager_id = user.id if user.role.name == "manager" else None
    return daily_analytics(db, manager_id=manager_id, date_from=date_from, date_to=date_to)
